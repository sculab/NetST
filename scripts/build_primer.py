...
class Extract_reference():
    ...
    def extract_reference_from_gb_parallel(self):
        ...
        if self.split_only == False:
            with open(os.path.join(out_dir,"seq_results.csv"), "w", newline='') as csv_file: pass
        ...
        if thread_number > 1:
            for key, value in my_records_ultimate.items():
                ...
                task_pool2.append(executor2.submit(self.write_fasta_file, value, files_count, os.path.join(out_dir, key + ".Fasta")))
            for i in task_pool2:
                results2.append(i.result())
        else:
            for key, value in my_records_ultimate.items():
                path = os.path.join(out_dir, key + ".fasta")
                results2.append(self.write_fasta_file(value, files_count, os.path.join(out_dir, key + ".Fasta")))
...
if __name__ == '__main__':
    ...
    pars.add_argument('-input', metavar='<str>', type=str,
                      help='''input folder.''', required=False, default='./alignment.fasta')
    ...
    pars.add_argument('-t', metavar='<int>', type=int,
                      help='''thread number''', required=False, default=16)
    pars.add_argument('-usevar', type=str2bool, nargs='?', const=True, help='''Designing primers using consensus region only''', default = True)
    pars.add_argument('-split_only', type=str2bool, nargs='?', const=True, help='''是否仅仅分隔序列''', default = True)
    pars.add_argument('-out_dir', metavar='<str>', type=str,
                      help='''output folder.''', required=False, default='./out_test')
