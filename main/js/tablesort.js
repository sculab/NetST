    ...
    Plugin.prototype = {
        ...
        setup: function (table, tableSortContainer) {
            ...
            function newRow(arr) {
                var row = $('<tr></tr>');
                for (var i = 0; i < arr.length; i++) {
                    var td = $('<td>' + arr[i] + '</td>').css('word-break', 'break-all');
                    row.append(td);
                }
                return row;
            }
            ...
            //table.find('th:not(:last-child)').css('width', '100px');
            //table.find('td:not(:last-child)').css('width', '100px');
            table.find('td').css('word-break', 'break-all');
        ...
        search: function (table, tableSortContainer) {
            ...
            function filter(options) {
                ...
                if (searchTerm.length > 1) {
                    ...
                    tr.each(function () {
                        var el = $(this);
                        var matchText = el
                            .find('td:not(:last-child)') // 修改这里，不包括最后一列
                            .text()
                            .replace(/\n\s+/g, ' ');
                        ...
                        if (matchText.match(match) !== null) {
                            ...
                            el.find('td:last-child').html(function (index, oldHtml) {
                                return oldHtml.replace(/<span class="table-sort-highlight">|<\/span>/g, ''); // 移除最后一列的已添加的标签
                            });
                            el.show();
                        } else {
                            el.hide();
            ...
            if (table.hasClass('table-sort-search')) {
                ...
                var search = $('<div class="table-sort-search-container"><input class="table-sort-search-input" type="text" placeholder="请输入要筛选的文字..."></div>');
