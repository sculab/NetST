...
$(function () {
    ...
    if (typeof nd !== 'undefined') {
      ...
      if (typeof og !== 'undefined') {
        if (typeof ng !== 'undefined') {
          ...
          nodeList[nd].proportions.push({ color: '#' + newcolor, group: newgroup, radius: nodeList[nd].radius, value: haplocount, pattern: newpattern, timecolor: haplotimecolor, nodestyle: haplostyle });
          nodeList[nd].proportions[og].value -= haplocount;
        }
        ...
        if (svg) {
          ...
          var path = n.selectAll('path').remove();
          path = n.selectAll('path').data(function (d) { return pie(d.proportions); });
          ...
          path.enter()
			  ...
			  .attr('d', function (d) {
				if (isINNerNode(d)) {
				  ...
				  if (styleid === 1) return sector(d);
				  else if (styleid === 2) return sectorNull(d);
				  else return sectorOuter(d);
				}
			  })
			...
			path.enter()
			  ...
			  .attr('d', function (d) {
				if (isINNerNode(d)) {
				  ...
				  if (styleid === 1) return sectorNull(d);
				  else if (styleid === 2) return sector(d);
				  else return sectorSmall(d);
				}
			  })
			  .style('fill', function (d) {
  ...
  function updateSVG() {
    ...
    node.style('stroke-width', function () { return linewidth * 2; }).style('stroke', '#000000').style('fill', '#000000');
    node.exit().remove();
  ...
  function getGroupsGrid(style) {
    ...
    $().w2grid({
      ...
      columns: [
        {
          field: 'recid', caption: '分组', size: '45%', sortable: false, resizable: false, editable: { type: 'text' },
          render: function (r) {
            return '<div style="font-weight: bold">' + r.recid + '</div>';
          }
        },
        {
          field: 'color', caption: '颜色', size: '20%', sortable: false, resizable: false, editable: { type: 'color' },
          render: function (r) {
            return '<div style="background-color: #' + r.color + '">&nbsp;</div>';
          }
        },
        {
          field: 'pattern', caption: '样式', size: '35%', sortable: false, resizable: false, editable: { type: 'combo', items: pattern_names, filter: false },
          render: function (r) {
      ...
      toolbar: {
        items: [
          { type: 'button', id: 'add_group', caption: '添加', icon: 'w2ui-icon-plus' },
  ...
  function getHaplotypesGrid(style) {
    ...
    $().w2grid({
      ...
      columns: [
        ...
        {
          field: 'group', text: '分组', size: '45%', sortable: true, resizable: true, editable: { type: 'combo', items: ['Default'], filter: false },
          render: function (r) {
  ...
  function getLayout(style, groups, haplotypes) {
    $('#layout').w2layout({
      ...
      padding: 0,
      panels: [
        //{ type: 'top', size: 0, resizable: false, style: style },
        {
          type: 'left', size: 250, maxSize: 350, resizable: true, title: '数据', style: style,
          tabs: {
            name: 'tabs',
            active: 'tab1',
            tabs: [
        ...
        { type: 'right', size: 140, resizable: false, title: '设定', style: style },
        {
          type: 'main', size: '100%', overflow: 'hidden', style: style,
          toolbar: {
            items: [
              { id: 'btn-svgsave', type: 'button', text: '保存 SVG', icon: 'icon-file-svg', disabled: true },
              //{ id: 'btn-pdfsave', type: 'button', text: 'Save PDF', icon: 'icon-file-pdf-o', disabled: true },
              { type: 'break' },
              { id: 'btn-zoomin', class: 'zoom-btn', type: 'button', text: '放大', icon: 'icon-zoom-in', disabled: true },
              { id: 'btn-zoomout', class: 'zoom-btn', type: 'button', text: '缩小', icon: 'icon-zoom-out', disabled: true },
              { type: 'break' },
              { id: 'btn-delnode', type: 'check', text: '删除节点', icon: 'icon-delete-node', disabled: true, checked: false },
              { id: 'btn-dellink', type: 'check', text: '删除链接', icon: 'icon-delete-link', disabled: true, checked: false },
              { type: 'break' },
              { id: 'btn-outline', type: 'check', text: '边框线', icon: 'icon-outline', disabled: true, checked: false },
              {
                id: 'btn-lwidth', type: 'menu', text: '线型', icon: 'icon-line-width', disabled: true,
                items: [
                  ...
                  { text: '4.0 px', lwidth: "4.0" },
                  { text: '5.0 px', lwidth: "5.0" },
                ]
              },
              {
                id: 'btn-time', type: 'menu', text: '圆心', icon: 'icon-circles-2', disabled: true,
                items: [
                  { text: 'Gray', typeid: "0" },
                  { text: 'Red', typeid: "1" },
                  { text: 'Green', typeid: "2" },
              ...
              {
                id: 'btn-style', type: 'menu', text: '风格', icon: 'icon-cross-4', disabled: true,
                items: [
                  { text: 'All', styleid: "0" },
              ...
              { id: 'btn-legend', type: 'check', text: '图例', icon: 'icon-legend', disabled: true, checked: false },
            ],
            onClick: function (e) {
              var target = e.target;
              switch (target) {
            ...
            if (labels.length > 0) {
              ...
              if (label !== '') {
                ...
                for (var j = 0; j < labels.length; j++) {
                  ...
                  /*
                   * Test if the first character is a digit. If so, prepend
                   * the label with 'L'.
                   */
  ...
  */
  ...
  function svgStart() {
    ...
    sectorOuter = d3.svg.arc()
      .outerRadius(function (d) { return d.data.radius * 1.4; })
      .innerRadius(function (d) { return d.data.radius * 0.7; });
	  
	sectorSmall = d3.svg.arc()
      .outerRadius(function (d) { return d.data.radius * 0.7; })
      .innerRadius(0);
    ...
    */
    ...
    w2ui.Layout_main_toolbar.enable('btn-dellink', 'btn-delnode', 'btn-svgsave', 'btn-outline', 'btn-lwidth', 'btn-zoomin', 'btn-zoomout', 'btn-legend', 'btn-time', 'btn-style');
  ...
  if (window.File && window.FileReader && window.FileList && window.Blob) {
    ...
    layout.html('right',
      ...
      '<div class="w2ui-field"><label>距离:</label><div><input type="text" id="linkDistance" /></div></div>' +
      '<div class="w2ui-field"><label>强度:</label><div><input type="text" id="linkStrength"  /></div></div>' +
      '<div class="w2ui-field"><label>摩擦:</label><div><input type="text" id="friction"  /></div></div>' +
      '<div class="w2ui-field"><label>耗费:</label><div><input type="text" id="charge"  /></div></div>' +
      // Do not mess with this
      //'<div class="w2ui-field"><label>Charge Distance:</label><div><input type="text" id="chargeDistance"  /></div></div>'+
      '<div class="w2ui-field"><label>重力:</label><div><input type="text" id="gravity" /></div></div>' +
      '<p>' +
      '<div style="text-align: center;"><button class="w2ui-btn" id="start" name="start" disabled>开始</button></div><p>' +
      '<div style="text-align: center;"><button class="w2ui-btn" id="stop" name="stop" disabled>停止</button></div>' +
      '</div>' +
  ...
  var event = {
    target: {
      files: [new File(["Test167.gml"], "Test167.gml")]
    }
  };

  // 调用loadGraph(event)函数并传递虚拟的事件对象
  loadGraph(gmlfile);
