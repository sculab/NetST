...
$(function () {
    ...
    function updateSVG() {
        ...
        node.enter().append('g')
            ...
            .attr('r', function (d) {
                if (d.nodestyle === 1) {
                    if (styleid === 1 || styleid === 2) return d.radius;
        ...
        Object.entries(labelNode).forEach(([key, value]) => {
            ...
            d3.select(selector)
                ...
                .attr('dx', function (d) {
                    if (d.nodestyle === 1) {
                        if (styleid === 1 || styleid === 2) return 5 + d.radius;
                        else return 5 + d.radius * 1.4;
                    } else {
                        return 5 + d.radius;
                    }
                })
                .attr('dy', '.35em')
                .text(value.join(";"))
                .style('font-family', 'Times New Roman, Times, serif') // Set the font family
                .style('font-size', '11px')
                .style('letter-spacing', '2px');// Increase font size for better readability
        });
    ...
    function getHaplotypesGrid(style) {
        ...
        $().w2grid({
            ...
            columns: [
                {field: 'recid', caption: 'ID', text: 'ID', size: '40%', sortable: true, resizable: true},
                {field: 'haplogroup', hidden: true},
                {field: 'color', hidden: true},
                {
                    field: 'group', caption: '分组', text: '分组', size: '24%', sortable: true, resizable: true,
                ...
                {
                    field: 'highlight', caption: '高亮', text: '高亮', size: '18%', resizable: true,
                    editable: false, // 禁止用户编辑
                    render: function (r) {
                        // 显示为复选框但禁用编辑
                ...
                {
                    field: 'label', caption: '标签', text: '标签', size: '18%', resizable: true,
                    editable: false, // 禁止用户编辑
                    render: function (r) {
                        // 显示为复选框但禁用编辑
    ...
    /*
        The characters grid
     */
    function getCharactersGrid(style) {
        ...
        $('body').append('<input id="loadCharacters" type="file" />');

        /*
         * After selecting a file, this triggers loadHaplotypes function
         */

        $('#loadCharacters').on('change', function (e) {
            loadCharacters(e);
        });
        ...
        $().w2grid({
            ...
            columns: [
                ...
                {field: 'traitDetail', caption: '性状类别', hidden: true}
            ],
            toolbar: {
                items: [
                    {type: 'button', id: 'load_characters', text: '加载', icon: 'icon-folder-open'},
                    {type: 'button', id: 'save_characters', text: '保存', icon: 'icon-file-save'}
                ],
                onClick: function (e) {
                    switch (e.target) {
                        case 'load_characters':
                            $('#loadCharacters').click();
                            break;
        ...
        // 如果网格已经创建，返回创建的网格对象
        if (w2ui.characters) return w2ui.characters;
        else return null;
    }
    ...
    */
                    ...
                    if (newnode) {
                        ...
                        if (labels.length > 0) {
                            ...
                            nodeList.push({
                                ...
                                id: haplogroup,
                                //  label holds the info which should be shown
                                label: []
                            });
                            /*
                        ...
                        if (name !== '') {                       // There is at least a label or name or something as a first field...
                            ...
                            if (k == -1) {
                                ...
                                if (typeof l[2] !== 'undefined') {   // read an optional pattern
                                    p = l[2].trim();
                                    j = pattern_names.filter(function (v, i) {
                                        return v.id === p;
                                    })[0];
    ...
    function loadCharacters(e) {
        ...
        var fileInput = document.getElementById('loadCharacters');
    ...
    */
    ...
    function loadHaplotypes(e) {
        ...
        if (w2ui.groups.records.length > 1) {  //Groups already loaded! Override?
            w2confirm('Colors will be replaced in the haplotype list! If<br>' +
                ...
                'Replace active group\'s list?').yes(function () {
                reader.readAsText(input.files[0]);
            });
        } else {
            reader.readAsText(input.files[0]);
    ...
    function highlightHaplotypes() {
        ...
        var nodeNames = Array.from(uniqueHaplogroupIds).map(function (id) {
            return getNameById(id);
        });
        // 更新全局变量
        highlightNode = nodeNames;
    ...
    if (window.File && window.FileReader && window.FileList && window.Blob) {
        ...
        var characters = getCharactersGrid(style);

        /*
         * Create a new layout
         */

        var layout = getLayout(style, groups, haplotypes, characters);
